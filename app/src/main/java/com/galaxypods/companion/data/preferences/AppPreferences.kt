...
class AppPreferences
    ...
    constructor(
        ...
        val autoPauseEnabled: Flow<Boolean> =
            context.dataStore.data
                .map { it[KEY_AUTO_PAUSE_ENABLED] ?: true }

        val autoPauseMode: Flow<AutoPlayPause.Mode> =
            context.dataStore.data
                .map { prefs ->
        ...
        suspend fun setOnboardingCompleted(value: Boolean) = update(KEY_ONBOARDING_COMPLETED, value)

        suspend fun setAutoPauseEnabled(value: Boolean) = update(KEY_AUTO_PAUSE_ENABLED, value)

        suspend fun setAutoPauseMode(mode: AutoPlayPause.Mode) = update(KEY_AUTO_PAUSE_MODE, mode.ordinal)

        suspend fun setVoiceAnnouncerEnabled(value: Boolean) = update(KEY_VOICE_ENABLED, value)

        suspend fun setVoiceAnnouncerThreshold(value: Int) = update(KEY_VOICE_THRESHOLD, value)

        suspend fun setLocationRecordEnabled(value: Boolean) = update(KEY_LOCATION_RECORD_ENABLED, value)

        suspend fun setCaseLostAlertEnabled(value: Boolean) = update(KEY_CASE_LOST_ENABLED, value)

        suspend fun setCrashlyticsOptIn(value: Boolean) = update(KEY_CRASHLYTICS_OPT_IN, value)
